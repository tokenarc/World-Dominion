...
const DEFAULT_STOCKS = [
  ...
  { symbol: "MEDICORP", name: "Medi Corp", sector: "healthcare", price: 95, volume: 700000 },
  { symbol: "AUTOIND", name: "Auto Industry", sector: "automotive", price: 120, volume: 850000 },
  { symbol: "MININGCO", name: "Mining Co", sector: "resources", price: 60, volume: 600000 },
  { symbol: "TELECOM", name: "TeleCom", sector: "telecomunications", price: 75, volume: 950000 },
  { symbol: "CONSTRUCT", name: "Construction Inc", sector: "construction", price: 55, volume: 400000 },
];
...
export const updateStockPrice = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const change24h = args.newPrice - stock.price;
    const percentChange = (change24h / stock.price) * 100;
    
    await ctx.db.patch(stock._id, {
      price: args.newPrice,
      change24h: Math.round(percentChange * 100) / 100,
      updatedAt: Date.now(),
    });
...
export const createListing = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const listingId = await ctx.db.insert("market_listings", {
  ...
  handler: async (ctx, args) => {
    ...
    if (args.itemType) {
      listings = await ctx.db
        ...
        .withIndex("itemType", q => q.eq("itemType", args.itemType).eq("status", "active"))
        .collect();
    } else {
      listings = await ctx.db
        .query("market_listings")
...
export const buyListing = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const buyQuantity = Math.min(args.quantity, listing.quantity);
    const totalCost = buyQuantity * listing.pricePerUnit;
    ...
    if (buyQuantity === listing.quantity) {
      ...
      await ctx.db.patch(listing._id, {
        quantity: listing.quantity - buyQuantity,
        totalPrice: (listing.quantity - buyQuantity) * listing.pricePerUnit,
        updatedAt: Date.now(),
      });
    }
    ...
    await ctx.db.insert("transactions", {
      ...
      amount: -totalCost,
      currency: "warBonds",
      description: `Purchased ${buyQuantity} ${listing.itemType}`,
      relatedId: listing._id.toString(),
      createdAt: Date.now(),
    });
    
    return { success: true, quantity: buyQuantity, totalCost };
  },
});
...
export const getMyListings = query({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db
      .query("market_listings")
      .withIndex("sellerId", q => q.eq("sellerId", player._id.toString()))
      .collect();
  },
});